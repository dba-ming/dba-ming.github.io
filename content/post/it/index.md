...
移位操作符的两个操作数必须是整型的。<span data-type="text" style="color: red;">整个移位表达式的值的类型也是整型的</span>，而且，左移位操作符与右移位操作符的运算并不对称。0111右移一位是把最后一位的1去掉，左边补个0，得0011，转换为十进制是3，这是正确的。并不等同于除以2 。
